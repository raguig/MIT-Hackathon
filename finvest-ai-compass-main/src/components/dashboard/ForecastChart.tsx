  ...
  const load = async () => {
    ...
    try {
      ...
      if (!r.series.length)
        toast({
          title: "No data",
          description: "Try another symbol or add your API key.",
        });
    } catch (e) {
      console.error(e);
      toast({
        title: "Market data error",
        description: "Check symbol or API key.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
      ...
      <div className="h-72 mt-3">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Tooltip
              formatter={(v) => (typeof v === "number" ? fmtCurrency(v) : v)}
        ...
        <div className="text-sm">
          ...
          <div className="flex gap-2 mt-1">
            <Input
              ...
              placeholder="Optional – save to query any symbol"
            />
            <Button
              onClick={() => {
                AlphaVantage.saveKey(apiKey);
                toast({
                  title: "Saved",
                  description: "API key stored locally",
                });
              }}
            >
              Save
          ...
          <div className="text-xs text-muted-foreground mt-1">
            Using demo key if empty (IBM only)
          </div>
        </div>
      </div>
    </Card>
